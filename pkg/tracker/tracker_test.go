...
func TestProjectObservation(t *testing.T) {
	...
	assert.NoError(t, po1.SetCallSign("cs1", false))
	...
	assert.NoError(t, po1.SetCallSign("cs2", true))
	assert.True(t, po1.HaveCallSign())
	...
	assert.NoError(t, po3.SetLocation(1.00, 2.00, false))
	...
	assert.NoError(t, po3.SetLocation(1.01, 2.01, true))
	...
	assert.NoError(t, po3.SetLocation(1.02, 2.02, true))
